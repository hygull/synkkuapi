...
func UserLogin(rw http.ResponseWriter, req *http.Request) {
	...
	fmt.Println("Referred URL : ",req.URL.Path);
...
/***************************Getting data from URLs *************************************/
	...
	// p_pic,err := strings.TrimSpace(req.Form["p_pic"][0]) //From google  (optional)
	// if err!=nil{
	// 		fmt.Println("You have not specified p_pic, but no problem")
...
/***************************************************************************************/
	fmt.Println("Extracted URL Parameters : ", umail,app_sign_in_key)
	...
	rows, err := db.Query("select  email_id from users where email_id= '" + umail + "' AND active=1;")
	if err != nil {
		panic(err.Error())
	}
	...
	if found {
		...
		views.ShowSuccessOrErrorAsJSON(rw,"EmailDoesNotExist","You have not downloaded, installed and registered your email for synkku application",142,42)
		return
	} /*else {
		timeNow := time.Now().String()[0:19]

<<<<<<< HEAD
		query := "insert into users(email_id, date_of_birth, city_id, gender, " +
			...
			" deleted_on ) values('"+ umail + "' , '1900-01-01',0, 0, 0, 'Default School', 'None', " +
=======
		query := "insert into users(first_name, last_name, email_id, date_of_birth, city_id, gender, " +
			" marital_status, studied_at, employed_at, community_id, profilepic_url, profilevideo_url, login_type, " +
			"account_type,stoken , stoken_updated_on, details_updated_on, updated_on, active, " +

			" deleted_on ) values(" + " '','' , '" + umail + "' , '1900-01-01',0, 0, 0, 'Default School', 'None', " +
			"0" + ", '" + "''" + "', '', 0, 1, '' , '1900-01-01 12:00:00', '" + timeNow + "' , '" + timeNow +

			" deleted_on ) values(" + "'" + "''" + "' , '" + umail + "' , '1900-01-01',0, 0, 0, 'Default School', 'None', " +
>>>>>>> 3a373073212f32f22a4e69bf53225f82e24f4b1e
			"0" + ", '" + "''"+ "', '', 0, 1, '' , '1900-01-01 12:00:00', '" + timeNow + "' , '" + timeNow +
...
func main(){
	...
	//To login 
	myRouter.HandleFunc("/index.php/feeds/userlogin", UserLogin); //DONE...stoken check is remaining

	if len(os.Args)>=2{
			fmt.Println("Server is listening on "+os.Args[1]);
			log.Fatal(http.ListenAndServe(":"+os.Args[1],myRouter));
	}else{
		    fmt.Println("You have not secified the PORT NUMBER. Run the command like this :>>  go run login.go 9000   etc")
		    os.Exit(1);
	}
